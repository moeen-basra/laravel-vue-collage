(function () {
    ...
    function setFrameAtCanvas(e) {
        fabric.Image.fromURL(e.target.getAttribute('data-src'), imageLoaderCallBack);
    }
    ...
    /*fabric.Image.fromURL('/assets/base/images/bg.png', function (img) {
    ...
    fabric.util.toArray(document.getElementsByClassName('frame'))
        .forEach(function (el) {
            el.onclick = setFrameAtCanvas;
        });
    ...
    $('send-backward').onclick = function () {
        sendBackward();
    };
    ...
    $('bring-forward').onclick = function () {
        bringForward();
    };

    $('remove-object').onclick = function () {
        removeObject();
    };
    ...
    var ctp = new CanvasToPNG('/user/save');

    document.getElementById('saveToPNG').onclick = function () {
        canvas.on('before:render', hideBg);
        canvas.on('after:render', showBg);

        canvas.deactivateAll();
        ctp.savePNG(canvas.toDataURL('png'), 'image');
